...
const (
	...
	harvesterUpgradeImageAnnotation = "harvesterhci.io/upgradeImage"
	repoInfoAnnotation              = "harvesterhci.io/repoInfo"
	singleNodeAnnotation            = "harvesterhci.io/singleNode"
...
func (h *upgradeHandler) OnChanged(key string, upgrade *harvesterv1.Upgrade) (*harvesterv1.Upgrade, error) {
	...
	if harvesterv1.UpgradeCompleted.GetStatus(upgrade) == "" {
		...
		if upgrade.Spec.Image == "" {
			...
			if _, err := repo.CreateImageFromISO(version.Spec.ISOURL); err != nil && !apierrors.IsAlreadyExists(err) {
				setUpgradeCompletedCondition(toUpdate, StateFailed, corev1.ConditionFalse, err.Error(), "")
				return nil, err
			}
		} else {
			...
			toUpdate.Annotations[harvesterUpgradeImageAnnotation] = upgrade.Spec.Image
			setImageReadyCondition(toUpdate, corev1.ConditionTrue, "", "")
		}
		harvesterv1.ImageReady.CreateUnknownIfNotExists(toUpdate)
		// setRepoProvisionedCondition(toUpdate, corev1.ConditionTrue, "", "")
		return h.upgradeClient.Update(toUpdate)
	}
	...
	if harvesterv1.RepoProvisioned.IsTrue(upgrade) && harvesterv1.NodesPrepared.GetStatus(upgrade) == "" {
		singleNodeName, err := h.isSingleNodeCluster()
		if err != nil {
			return nil, err
		}
		...
		if err != nil {
			toUpdate := upgrade.DeepCopy()
			toUpdate.Labels[upgradeStateLabel] = StateFailed
			harvesterv1.UpgradeCompleted.False(toUpdate)
			harvesterv1.UpgradeCompleted.Reason(toUpdate, err.Error())
			return h.upgradeClient.Update(toUpdate)
		}
		...
		toUpdate.Annotations[repoInfoAnnotation] = repoInfo
		if singleNodeName != "" {
			toUpdate.Annotations[singleNodeAnnotation] = singleNodeName
		}
	...
	if harvesterv1.SystemServicesUpgraded.IsTrue(upgrade) && harvesterv1.NodesUpgraded.GetStatus(upgrade) == "" {
		...
		singleNodeName := upgrade.Annotations[singleNodeAnnotation]
		if singleNodeName != "" {
...
func (h *upgradeHandler) upgradeKubernetes(kubernetesVersion string) error {
	...
	toUpdate.Spec.RKEConfig.UpgradeStrategy.ControlPlaneDrainOptions.DeleteEmptyDirData = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.ControlPlaneDrainOptions.Enabled = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.ControlPlaneDrainOptions.Force = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.ControlPlaneDrainOptions.IgnoreDaemonSets = &drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.WorkerDrainOptions.DeleteEmptyDirData = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.WorkerDrainOptions.Enabled = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.WorkerDrainOptions.Force = drain
	toUpdate.Spec.RKEConfig.UpgradeStrategy.WorkerDrainOptions.IgnoreDaemonSets = &drain

	// TODO: will hooks be called if not drained?? If not, how to deal with single node?
...
func getCachedRepoInfo(upgrade *harvesterv1.Upgrade) (*UpgradeRepoInfo, error) {
	...
	if err := repoInfo.Load(upgrade.Annotations[repoInfoAnnotation]); err != nil {
		return nil, err
	}
	return repoInfo, nil
}
