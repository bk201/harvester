...
func (r *UpgradeRepo) Bootstrap() error {
	upgradeImage, err := r.GetImage(r.upgrade.Annotations[harvesterUpgradeImageAnnotation])
	if err != nil {
		return err
	}
...
func getISODisplayNameImageName(version string) string {
	return fmt.Sprintf("harvester-%s", version)
}

func (r *UpgradeRepo) CreateImageFromISO(isoURL string) (*harvesterv1.VirtualMachineImage, error) {
	imageSpec := &harvesterv1.VirtualMachineImage{
		...
		Spec: harvesterv1.VirtualMachineImageSpec{
			DisplayName: getISODisplayNameImageName(r.upgrade.Spec.Version),
...
func (r *UpgradeRepo) createService() (*corev1.Service, error) {
	serviceName := fmt.Sprintf("%s%s", repoServiceNamePrefix, r.upgrade.Name)
...
func (r *UpgradeRepo) getInfo() (*UpgradeRepoInfo, error) {
	releaseURL := "http://10.10.0.1/harvester/iso/harvester-release.yaml"
