...
var _ = FDescribe("verify supportbundle APIs", func() {
	...
	Context("operate via steve API", func() {
		...
		Specify("verify required fields", func() {
			...
			By("creating a supportbundle without description", func() {
				...
				var sb = harvesterv1.SupportBundle{
		...
		Specify("verify supportbundle ready", func() {
			...
			MustFinallyBeTrue(func() bool {
				...
				Expect(harvesterv1.ImageInitialized.IsFalse(retSb)).NotTo(BeTrue())
				return harvesterv1.ImageInitialized.IsTrue(retSb)
