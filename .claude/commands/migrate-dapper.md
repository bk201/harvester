...
- No `MK_ENV_FILE` / version env file — all version computation happens inside the container via `./scripts/version`
...
5. **Git needed**: does it call `git checkout`, `git status`, etc.? (Not a problem — `.git` is copied in)
...
Note: `.git` is intentionally **not** excluded — the `base` stage needs it for `scripts/version` and dirty-check targets. `.gitignore` never lists `.git`, so copying it is safe.
