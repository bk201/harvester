...
upgrade_rancher()
{
  ...
  # This actually doesn't work very well because a deployed chart doesn't meant all pods are up and ready
  wait_helm_release cattle-fleet-system fleet fleet-100.0.3+up0.3.9-rc6 0.3.9-rc6 deployed
  wait_helm_release cattle-fleet-system fleet-crd fleet-crd-100.0.3+up0.3.9-rc6 0.3.9-rc6 deployed
  wait_helm_release cattle-system rancher-webhook rancher-webhook-1.0.4+up0.2.5-rc3 0.2.5-rc3 deployed
...
wait_kubevirt()
{
  ...
  echo "Waiting for KubeVirt $namespace/$name"
  echo "Target version: $version"

  while [ true ]; do
    kubevirt=$(kubectl get kubevirts.kubevirt.io $name -n $namespace -o yaml)
    current_phase=$(echo "$kubevirt" | yq e '.status.phase' -)
    current_version=$(echo "$kubevirt" | yq e '.status.observedKubeVirtVersion' -)
    echo "Current version: $current_version, Current phase: $current_phase"
...
upgrade_harvester()
{
  ...
  # TODO: skip if version doesn't change
...
EOF
  ...
  wait_kubevirt harvester-system kubevirt 0.49.0-2
}
...
# wait_until rancher
upgrade_harvester_cluster_repo
# wait_until cluster_repo
upgrade_harvester
# wait_until harvester
upgrade_monitoring
apply_extra_manifests
# wait_until end
